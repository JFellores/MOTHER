import { B as Bounds, V as Texture, W as TextureSource, X as NOOP, a as BufferUsage, c as BindGroup, dt as extensions, i as Buffer, it as Matrix, l as UniformGroup, o as Shader, p as GpuProgram, rt as groupD8, t as Geometry, ut as ExtensionType, x as GlProgram } from "./Geometry-CCNBtUnt.js";
import "./lib-B8Dedqfv.js";
import { c as ViewContainer, l as Container, n as State } from "./Filter-9Fdgh4OM.js";
//#region node_modules/@pixi/tilemap/lib/settings.mjs
var settings = {
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/TilemapGeometry.mjs
...
var _TilemapGeometry = class _TilemapGeometry extends Geometry {
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/TilemapPipe.mjs
...
var TilemapPipe = class {
	...
	*/
	...
	checkIndexBuffer(size) {
		...
		this.indexBuffer.data = createIndicesForQuads(size, settings.use32bitIndex ? new Uint32Array(totalIndices) : new Uint16Array(totalIndices));
	}
	...
	execute({ tilemap }) {
		...
		if (tilemap.compositeParent) anim_frame = tilemap.parent.tileAnim || anim_frame;
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/TileTextureArray.mjs
...
var TileTextureArray = class {
	constructor(max_textures) {
		...
		__publicField$4(this, "null_color", new Float32Array([
			0,
			0,
			0,
			.5
		...
		this.tex_buf = new Buffer({ usage: BufferUsage.UNIFORM | BufferUsage.COPY_DST });
	}
	get length() {
		return this.count;
	}
	...
	getBindGroup() {
		this.update();
		if (!this.dirty_gpu) return this.bind_group;
		...
		if (!this.bind_group) this.bind_group = new BindGroup(bind_group_resources);
	...
	static generate_gpu_textures(max_textures) {
		...
		for (let i = 0; i < max_textures; i++) src.push(`  case ${i}: { return textureSampleGrad(u_texture_${i}, u_sampler_${i}, uv, dx, dy); }`);
	...
	static generate_gl_textures(max_textures) {
		...
		for (let i = 0; i < max_textures; i++) src.push(`if(texture_id < ${i}.5) return texture(u_textures[${i}], uv * u_texture_size[${i}].zw);`);
	...
	static gl_gen_resources(max_textures) {
		const sampleValues = [];
		for (let i = 0; i < max_textures; i++) sampleValues[i] = i;
		const samplerSize = [];
		for (let i = 0; i < max_textures; i++) {
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/gl_tilemap.mjs
...
var GlTilemapAdaptor = class extends TilemapAdaptor {
	...
	execute(pipe, tilemap) {
		...
		for (let i = 0; i < tileset.length; i++) renderer.texture.bind(tileset.arr[i], i);
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/gpu_tilemap.mjs
...
var GpuTilemapAdaptor = class extends TilemapAdaptor {
	...
	init() {
		this._shader = new Shader({ gpuProgram: GpuProgram.from({
			vertex: {
				source: gpu_tilemap_vertex,
				entryPoint: "mainVert"
			},
			fragment: { source: gpu_tilemap_fragment.replace("//include_textures", TileTextureArray.generate_gpu_textures(this.max_textures)) }
		}) });
		this.bind_group = new BindGroup({ ut: this.pipe_uniforms });
	}
};
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/Tilemap.mjs
...
var Tilemap = class extends ViewContainer {
	/**
	...
	*/
	constructor(tileset) {
		...
		*/
		__publicField$1(this, "shadowColor", new Float32Array([
			0,
			0,
			0,
			.5
	...
	*/
	setTileset(textureOrArray = []) {
		...
		if (textureOrArray instanceof TileTextureArray) {
			...
			if (ts.count === 1 && ts.arr[0] === textureOrArray) return this;
			...
			if (textureOrArray.length === ts.count) {
				let flag = true;
				for (let i = 0; i < textureOrArray.length; i++) if (textureOrArray[i]?.source !== ts.arr[i]) {
					flag = false;
					break;
				}
				if (flag) return this;
			...
			ts = this.tileset = new TileTextureArray(settings.TEXTURES_PER_TILEMAP);
			for (let i = 0; i < textureOrArray.length; i++) ts.push(textureOrArray[i]?.source);
			this.didViewUpdate = true;
		}
		return this;
	}
	...
	*/
	tile(tileTexture, x, y, options = {}) {
		...
		if (typeof tileTexture === "number") {
			...
			let texture;
			if (typeof tileTexture === "string") texture = Texture.from(tileTexture);
			else texture = tileTexture;
			const textureList = this.tileset;
			for (let i = 0; i < textureList.count; i++) if (textureList.arr[i] === texture.source) {
				textureIndex = i;
				break;
			}
			if ("frame" in texture) {
	...
	updateBuffer(plugin) {
		...
		let vb = this.vb;
		if (this.tileset.count === 0 || rects_count === 0 || this.rects_count === rects_count && vb) return;
		...
		for (let i = 0; i < points.length; i += POINT_STRUCT_SIZE) {
			const eps = .5;
			if (this.compositeParent) textureId = points[i + 9];
			const x = points[i + 2];
			const y = points[i + 3];
			const w = points[i + 4];
			const h = points[i + 5];
			...
			const animDivisor = points[i + 12];
			const alpha = points[i + 13];
			let u0;
			let v0;
			let u1;
			let v1;
	...
	*/
	isModified(anim) {
		if (this.rects_count * POINT_STRUCT_SIZE !== this.pointsBuf.length || anim && this.hasAnimatedTile) return true;
		return false;
	}
	/**
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/CompositeTilemap.mjs
...
var CompositeTilemap = class extends Container {
	...
	*/
	tile(tileTexture, x, y, options = {}) {
		...
		if (typeof tileTexture === "number") {
			...
			if (typeof tileTexture === "string") tileTexture = Texture.from(tileTexture);
			for (let i = 0; i < children.length; i++) {
				const child = children[i];
				const tex = child.getTileset().arr;
				for (let j = 0; j < tex.length; j++) if (tex[j] === tileTexture.source) {
					tilemap = child;
					break;
				}
				if (tilemap) break;
	...
	*/
	isModified(anim) {
		...
		for (let i = 0; i < layers.length; i++) if (layers[i].isModified(anim)) return true;
		return false;
	}
	/**
	* @internal
...
//#endregion
//#region node_modules/@pixi/tilemap/lib/index.mjs
extensions.add(TilemapPipe);
extensions.add(GlTilemapAdaptor);
extensions.add(GpuTilemapAdaptor);
//#endregion
export { CompositeTilemap, Constant, POINT_STRUCT_SIZE, Tilemap, TilemapAdaptor, TilemapGeometry, TilemapPipe, settings };
